...
export class FriendshipService {
  ...
  async createFriendship(data: CreateFriendshipDto): Promise<void> {
    ...
    await this.apiGatewayClient.notifyUsers([data.requester_id, data.addressee_id], event);
  }
  ...
  async deleteFriendship(data: DeleteFriendshipDto): Promise<void> {
    ...
    await this.apiGatewayClient.notifyUsers([friendship.requester_id, friendship.addressee_id],event);
  }

  // Cancel a pending outgoing request — only the original requester may do this
  async cancelFriendshipRequest(data: {
    ...
    await this.apiGatewayClient.notifyUsers(
  ...
  async getFriendshipBetween(data: FriendshipDto): Promise<Friendship | null> {
    return this.friendshipRepository.findBetween({
  ...
  async getAllByStatusForUser(
    data: FindAllByStatusForUserDto
  ): Promise<Friendship[]> {
    return this.friendshipRepository.findAllByStatusForUser({
      userId: data.userId,
      status: data.status
    });
  }
  ...
  async isBlockedBy(data: IsBlockedDto): Promise<boolean> {
    return this.friendshipRepository.isBlockedBy({
  ...
  async blockUser(data: BlockUserDto): Promise<Friendship> {
    ...
    return this.friendshipRepository.blockUser({
      blocker_id: data.blocker_id,
  ...
  async unblockUser(data: {
    ...
    const friendship = await this.friendshipRepository.findDirectionalBlock(
      data.blocker_id,
      data.blocked_id
    );
    if (!friendship) {

...
export async function registerTournamentRoutes(
  ...
  server.post(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      if (
        ...
        (!Number.isInteger(body.maxPlayers) || body.maxPlayers < 2)
      ) {
        return reply.status(400).send({
          error: 'Bad Request',
      ...
      if (
        body.matchDeadlineMin != null &&
        (typeof body.matchDeadlineMin !== 'number' ||
          body.matchDeadlineMin <= 0)
      ) {
        return reply.status(400).send({
          error: 'Bad Request',
  ...
  server.get(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      try {
        const tournament =
          await tournamentService.getTournamentSummary(tournamentId);
  ...
  server.post(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      try {
        const { full } = await tournamentService.register(
          tournamentId,
          userId,
          username
        );
        ...
        if (full) {
          lifecycleManager?.onRegistrationFull(tournamentId).catch(err => {
            request.log.error(
              ...
              'Error handling registration full'
            );
          });
        }
  ...
  server.post(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      try {
        ...
        request.log.info(
          { tournamentId, userId },
          'User unregistered from tournament'
        );
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      try {
        const participantIds =
          await tournamentService.getParticipantIds(tournamentId);
